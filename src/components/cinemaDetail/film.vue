	...
	<section class="pr v-slide-content">
		<ul class="v-slide-box pa" ref='vSlide' :class="{'slide-action': true}">
			<li v-for="item in lists">
				<img style="height: 90px" :class="{'click-select': lists[0] === item}" :src="item.moviePicture" :data-id="item.id">
			</li>
		</ul>
	</section>
</template>
...
export default{
    ...
    ...mapMutations([
			...
			Array.prototype.forEach.call(imgEles, (item) => {
				item.onload = function () {
					...
					if (loadNumber === content.imgNumber) {
						...
						slideBox.style.width = slideBoxLength +content.lists.length*15+'px'
					}
				}
			})
		},
      ...
      console.log("点击了")
        ...
        this.$store.commit('updateMovieid',event.target.getAttribute('data-id'));
			}, false)

<template>
  <section id='coming'>
    <section v-for="comingItem in comingLists">
      <p class="open-date">{{ comingItem.releaseTime }} {{ comingItem.day }}</p>
      <ul class="coming">
        <li :class="{'border-bottom': comingItem.movies[comingItem.movies.length-1] != item}" v-for="(item, index) in comingItem.movies" :key="item.id">
          <figure class="m-img ml fl pr" @click="playMovieVideo(item.prevideo)">
            <img v-lazyload="item.moviePicture">
          ...
          <div class="mr">
            <div class="m-buy fr">
              <router-link :to="{ name: 'MovieCinema', params: { id: item.id }}">
            ...
            <div class="m-info">
              <router-link :to="{ name: 'detail', params: { id: item.id }}">
                <div class="m-name">
                  <h2 class="f16 tddd vm fl">{{ item.movieName }}</h2>
                </div>
                <p>类型：{{ item.movieType }}</p>
                <p>简介：{{ item.discription }}</p>
              </router-link>
            </div>
          </div>
        </li>       

	...
	<section id="movie-detail">
		<header class="mbg pr">
			<figure class="d-cover">
				<img :src="detailObj.moviePicture ? detailObj.moviePicture : 'https://gw.alicdn.com/tps/i1/TB147JCLFXXXXc1XVXXxGsw1VXX-112-168.png'" alt="">
			</figure>
			<article class="d-info">
				<p class="d-name tddd">{{ detailObj.movieName }}</p>
				<!--<p class="tddd">{{ detailObj.showNameEn }}</p>-->
				<p>{{ detailObj.movieType }}</p>
				<p>{{ detailObj.location }}｜{{ detailObj.runningTime }}分钟</p>
				<p>{{ detailObj.releaseTime }}在中国上映</p>
				<div class="full-star pr">
					<div class="score-start" :style="{width: `${detailObj.score * 10}%`}"></div>
					<span class="score pa">{{ detailObj.score }}分</span>
				</div>
			</article>
		</header>
		<section v-if="infoObj">
			...
			<section class="ab-photo pr border-1px-bottom">
				...
				<div class="swiper-container">
				    <div class="swiper-wrapper" v-if="detailObj.actorList">
				        <div class="swiper-slide" v-for="item in detailObj.actorList">
				        	<div class="am-img pr">
				        		<img :src="item.actorPicture" alt="">
				        		<div class="am-title pa">
				        			<p class="tddd">{{ item.actorName }}</p>
				        			<!--<p>{{ item.profession }}</p>-->
				        		</div>
				        	</div>
				        </div>
			...
			<section class="ab-photo crew-ab-photo pr border-1px-bottom">
				...
				<div class="swiper-container crew-swiper-container">
				    <div class="swiper-wrapper">
				        <div class="swiper-slide" v-for="item in stagePhotos">
			...
			<hotEvaluation :lists="commentList"></hotEvaluation>
		</section>
...
export default{
	...
	created () {
		...
		//根据电影id查询评论
    this.$axios.get('/comment/commentspage/'+id).then(({data}) => {
      this.commentList = data;
      //console.log(JSON.stringify(this.commentList))
    })

		//根据电影id查询详情
    this.$axios.get('/movie/getMovieDetail/'+id).then(({data})=>{
      //console.log(data)
      this.detailObj = data;
      //分割剧照
      this.stagePhotos = data.stagePhotos.split(",");
      //console.log(this.stagePhotos);
      //评论
      this.evalLists = data.commentList;
